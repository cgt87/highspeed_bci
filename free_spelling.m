...
% -------------------------------------------------------------------------
% 与字符输入系统连接
tChar = tcpip('192.168.1.254',31500);
fopen(tChar);
% -------------------------------------------------------------------------
% 实时数据传输时的参数设置
params.chanNum = 9;
...
try
    ...
    for ii = 1:trialnum
        ...
        if resultnum>=1&&resultnum<=26
            CharRes = ['BCIID01CH00',char(resultnum+64)];
        elseif resultnum>=27&&resultnum<=36
             CharRes = ['BCIID01CH00',char(resultnum+21)];
        elseif resultnum==37
            CharRes = ['BCIID01CH00',char(63)];
        elseif resultnum==38
            CharRes = ['BCIID01CH00',char(44)];
        elseif resultnum==39
            CharRes = ['BCIID01CH00',char(46)];
        elseif resultnum==40
            CharRes = 'BCIID01CH111';
        end
        fwrite(tChar,CharRes);
        % -----------------------------------------------------------------
        if resultnum==40
            showresult(end)=[];
        else
